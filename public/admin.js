...
class AdminModule {
        ...
        return `
            <div class="admin-card" style="border-left-color: ${statusInfo.color};">
                ...
                <div class="admin-card-actions">
                    ...
                    ${r.status === 'pending' ? `
                        <button type="button" class="admin-btn admin-btn-danger" onclick="dashboard.modules.admin.cancelRequest(${r.id})">
    ...
    showRequestModal(r) {
        ...
        if (this.modalBody) {
            this.modalBody.innerHTML = `
                ...
                <div class="admin-modal-row"><strong>Заявитель:</strong> ${this.escapeHtml(r.teacher_name)}</div>
                <div class="admin-modal-row"><strong>Замещающий:</strong> ${r.replacing_teacher ? this.escapeHtml(r.replacing_teacher) : '—'}</div>
    ...
    async cancelRequest(id) {
        if (!id) return;
        if (!confirm('Отменить эту заявку? Доступно только для статуса «На рассмотрении».')) return;
